...
# IMPORTANT: please download the kickstarter zip file from this link:
# https://www.kaggle.com/codename007/funding-successful-projects/downloads/funding-successful-projects-on-kickstarter.zip/1
# afterdownloading, please place the "train.csv" and "test.csv" files in your working directory of Rstudio, alternatively go to my
# GitHub repository that is added in the comments and donwload the two files to your working directory
# There is no easy way to download the dat set from the website as it requires a user name and password. I assume not every one has a
# Kaggle account which is why I propose you download it from their site or my GitHub repository

# The zip file that can be downloaded from the link above contains a test set, training set and a sample submission set
...
getwd() # return the working directory which is the directory in which you have to put the "test.csv" and "train.csv" files for the code to run
kickstarter <- read_csv("~/train.csv") # loading of the test file
# REMINDER: if the train.csv and test.csv are not in your working directory, the code will not run correctly
...
kickstarter_test <- read_csv("~/test.csv")
...
concur <- concur %>% rename("US"=X.USD.,"GB"=X.GBP.,"CA"=X.CAD.,"AU"=X.AUD.,"NZ"=X.NZD.,"NL"=X.EUR.,
                            "SE"=X.SEK.,"IE"=X.EUR..1,"NO"=X.NOK.,"DK"=X.DKK.,"DE"=X.EUR..2)
concur
